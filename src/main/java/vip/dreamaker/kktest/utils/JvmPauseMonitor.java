...
public class JvmPauseMonitor {
    ...
    public JvmPauseMonitor(long warnThresholdMs, long infoThresholdMs, String name) {
        this.warnThresholdMs = warnThresholdMs;
        this.infoThresholdMs = infoThresholdMs;
        this.name = name;
    }

    public JvmPauseMonitor(String name) {
    ...
    public static void main(String[] args) throws Exception {
        new JvmPauseMonitor(5, 3,"kktest").start();
    ...
    private class Monitor implements Runnable {
        ...
        @Override
        public void run() {
            TaggedMetricRegistry taggedMetricRegistry = TaggedMetricRegisterProvider.getInstance().getTaggedMetricRegistry();
            Histogram jvmPauseHistogram = taggedMetricRegistry.histogram("vip.dreamaker.jvm.pause." + name);
            Histogram gcCountHistogram = taggedMetricRegistry.histogram("vip.dreamaker.gc.count." + name);
            Histogram gcTimesHistogram = taggedMetricRegistry.histogram("vip.dreamaker.gc.times." + name);
            ...
            while (shouldRun) {
                ...
                jvmPauseHistogram.update(extraSleepTime);
                Pair<Long, Long> gcPair = buildGcPair(gcTimesAfterSleep, gcTimesBeforeSleep);
                gcCountHistogram.update(gcPair.getKey());
                gcTimesHistogram.update(gcPair.getValue());
    ...
    private Pair<Long, Long> buildGcPair(Map<String, GcTimes> gcTimesAfterSleep, Map<String, GcTimes> gcTimesBeforeSleep) {
        Set<String> gcBeanNames =
                Sets.intersection(gcTimesAfterSleep.keySet(), gcTimesBeforeSleep.keySet());
        long gcCount = 0;
        long gcTimes = 0;
        for (String name : gcBeanNames) {
            ...
            gcTimes += diff.gcTimeMillis;
        }
        Pair<Long, Long> gcPair = new Pair<>(gcCount, gcTimes);
        return gcPair;
    }
}
